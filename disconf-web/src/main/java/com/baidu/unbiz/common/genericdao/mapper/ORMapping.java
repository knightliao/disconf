...
/**
 * 此對象主要是一個存儲結構，存儲的是實體對象和數據庫表的映射關係的數據 該表被QueryGenerater應用作為生成Query的數據基礎
 * 該表被GenericMapper應用作為裝載實體對象的規則
 * 
...
public class ORMapping<ENTITY extends BaseObject<K>, K extends Serializable> {
	...
	private void initMapping() {
		...
		// 此处增加处理以便零注解
		this.table = (table.name().equals("")) ? table.columnStyle().convert(
				entityClass.getSimpleName()) : table.name();
		this.table = table.db() + "." + this.table;

		this.shardCount = table.shardCount();

		List<MappingItem> mappingItems = MappingItem
				.getMappingItems(entityClass);
